package httpserver

import (
	"bytes"
	"context"
	"image"
	"image/color"
	"image/png"
	"os"
	"path/filepath"
	"testing"

	"github.com/vndroid/istore/internal/options"
	"github.com/vndroid/istore/internal/ossprocess"
	"github.com/vndroid/istore/internal/source"
)

// newTestProvider returns a provider over a temp root holding one 4x4 PNG named
// wm.png.
func newTestProvider(t *testing.T) *watermarkProvider {
	t.Helper()

	dir := t.TempDir()

	img := image.NewRGBA(image.Rect(0, 0, 4, 4))
	for x := range 4 {
		for y := range 4 {
			img.Set(x, y, color.RGBA{R: 255, A: 255})
		}
	}
	var buf bytes.Buffer
	if err := png.Encode(&buf, img); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(dir, "wm.png"), buf.Bytes(), 0o600); err != nil {
		t.Fatal(err)
	}

	src, err := source.NewLocal(dir)
	if err != nil {
		t.Fatal(err)
	}

	// No TTL: a local source's cached entries never expire, which is the
	// behaviour these tests are about.
	return newWatermarkProvider(src, NewDefaultConfig(), 0)
}

func imageOptions(path string) *options.Options {
	o := options.New()
	o.Set(ossprocess.KeyWatermarkPath, path)
	return o
}

func textOptions(text string) *options.Options {
	o := options.New()
	o.Set(ossprocess.KeyWatermarkText, text)
	return o
}

func (p *watermarkProvider) size() int {
	p.mu.Lock()
	defer p.mu.Unlock()
	return len(p.loaded)
}

func TestWatermarkTextCacheKeyCoversRenderingOptions(t *testing.T) {
	base := watermarkSpec{path: "wm.png", text: "hello", font: "sans", size: 40}
	keys := []string{base.cacheKey()}
	for _, change := range []func(*watermarkSpec){
		func(s *watermarkSpec) { s.path = "other.png" },
		func(s *watermarkSpec) { s.text = "goodbye" },
		func(s *watermarkSpec) { s.font = "serif" },
		func(s *watermarkSpec) { s.color.R = 1 },
		func(s *watermarkSpec) { s.size = 41 },
		func(s *watermarkSpec) { s.shadow = .5 },
		func(s *watermarkSpec) { s.rotate = 1 },
		func(s *watermarkSpec) { s.order = 1 },
		func(s *watermarkSpec) { s.align = 1 },
		func(s *watermarkSpec) { s.interval = 1 },
	} {
		s := base
		change(&s)
		keys = append(keys, s.cacheKey())
	}
	seen := make(map[string]bool)
	for _, key := range keys {
		if seen[key] {
			t.Fatalf("distinct watermark specs produced the same cache key: %q", key)
		}
		seen[key] = true
	}
}

// TestWatermarkImageIsCached is the reason the map exists: a site's one
// watermark is read off disk once, not per request.
func TestWatermarkImageIsCached(t *testing.T) {
	p := newTestProvider(t)
	defer p.Close()

	first, _, err := p.Get(context.Background(), imageOptions("wm.png"))
	if err != nil {
		t.Fatalf("first Get: %v", err)
	}
	defer first.Close()

	second, _, err := p.Get(context.Background(), imageOptions("wm.png"))
	if err != nil {
		t.Fatalf("second Get: %v", err)
	}
	defer second.Close()

	if first != second {
		t.Error("second Get returned a different instance; the image was rebuilt")
	}
	if got := p.size(); got != 1 {
		t.Errorf("cache holds %d entries, want 1", got)
	}
}

// A successful text render is cached under its parameter digest. A test build
// without initialized libvips cannot render it and must leave no failed entry.
func TestWatermarkTextCache(t *testing.T) {
	p := newTestProvider(t)
	defer p.Close()

	first, _, err := p.Get(context.Background(), textOptions("repeat"))
	if err != nil {
		if got := p.size(); got != 0 {
			t.Errorf("failed text render left %d entries", got)
		}
		return
	}
	defer first.Close()
	second, _, err := p.Get(context.Background(), textOptions("repeat"))
	if err != nil {
		t.Fatal(err)
	}
	defer second.Close()
	if first != second || p.size() != 1 {
		t.Fatal("repeated text watermark was not cached")
	}
}

func TestWatermarkMixedTextCache(t *testing.T) {
	p := newTestProvider(t)
	defer p.Close()

	o := imageOptions("wm.png")
	o.Set(ossprocess.KeyWatermarkText, "repeat")
	first, _, err := p.Get(context.Background(), o)
	if err != nil {
		if got := p.size(); got != 0 {
			t.Errorf("failed mixed render left %d entries", got)
		}
		return
	}
	defer first.Close()
	second, _, err := p.Get(context.Background(), o)
	if err != nil {
		t.Fatal(err)
	}
	defer second.Close()
	if first != second || p.size() != 1 {
		t.Fatal("repeated mixed watermark was not cached")
	}
}

func TestWatermarkNoneRequested(t *testing.T) {
	p := newTestProvider(t)
	defer p.Close()

	d, _, err := p.Get(context.Background(), options.New())
	if err != nil {
		t.Fatalf("Get: %v", err)
	}
	if d != nil {
		t.Error("Get returned an image for a request with no watermark")
	}
}

func TestWatermarkMissingObjectKey(t *testing.T) {
	p := newTestProvider(t)
	defer p.Close()

	if _, _, err := p.Get(context.Background(), imageOptions("nope.png")); err != ErrBadWatermark {
		t.Errorf("Get = %v, want %v", err, ErrBadWatermark)
	}
	if got := p.size(); got != 0 {
		t.Errorf("cache holds %d entries after a failed build, want 0", got)
	}
}

// TestWatermarkTraversal: an object key is resolved through the same resolver as
// the image being served, so it cannot escape the root either.
//
// Note what a `..` key does: the resolver anchors at the root and cleans, so
// "../../etc/passwd" is the root's own etc/passwd, which does not exist. It is
// not an escape and never was — the assertion is that it stays inside.
func TestWatermarkTraversal(t *testing.T) {
	p := newTestProvider(t)
	defer p.Close()

	for _, key := range []string{"../../etc/passwd", "/etc/passwd", "../../../../../../etc/hosts"} {
		if _, _, err := p.Get(context.Background(), imageOptions(key)); err != ErrBadWatermark {
			t.Errorf("Get(%q) = %v, want %v", key, err, ErrBadWatermark)
		}
	}
}
